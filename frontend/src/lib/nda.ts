// Domain model for the Mutual NDA creator (PL-2).
//
// The Mutual NDA is built from two parts:
//   1. A Cover Page holding all user-editable "key information".
//   2. Fixed "Standard Terms" boilerplate (Common Paper Mutual NDA v1.0).
//
// Source templates live in the repo's `templates/` directory
// (Mutual-NDA-coverpage.md and Mutual-NDA.md), free to use under CC BY 4.0.
...
export type NdaTermKind = "expires" | "untilTerminated";
...
/** Human-readable effective date, falling back to a placeholder while empty. */
...
export function describeNdaTerm(data: NdaFormData): string {
  return data.ndaTermKind === "untilTerminated"
    ? "Continues until terminated in accordance with the terms of the MNDA."
    : `Expires ${yearLabel(data.ndaTermYears)} from the Effective Date.`;
}

export function describeConfidentiality(data: NdaFormData): string {
  return data.confidentialityKind === "perpetuity"
    ? "In perpetuity."
    : `${yearLabel(
...
/** A field value or a bracketed placeholder when the user hasn't filled it in. */
export function orPlaceholder(value: string, placeholder: string): string {
  const trimmed = value.trim();
  return trimmed.length > 0 ? trimmed : `[${placeholder}]`;
}

// The Standard Terms are fixed boilerplate and are reproduced verbatim from
// the Common Paper Mutual NDA Standard Terms v1.0.
